...
<body class="bg-cream min-h-screen"
    x-data="{ sidebarOpen: false, showAddModal: false, showEditModal: false, editId: null, editName: '' }">
    <div class="flex">
        <!-- Sidebar -->
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <nav class="p-4 space-y-2">
                <a href="{{ route('admin.dashboard') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl text-charcoal/70 hover:bg-cream transition">
                ...
                <a href="{{ route('admin.categories.index') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl bg-earthy/10 text-earthy font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-charcoal/10">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center justify-center gap-2 text-charcoal/70 hover:text-charcoal py-2 rounded-xl hover:bg-cream transition text-sm">
        ...
        <div class="flex-1 min-h-screen">
            <header class="h-20 bg-white border-b border-charcoal/10 flex items-center justify-between px-6">
                ...
                <button @click="showAddModal = true"
                    class="bg-earthy text-cream px-5 py-2.5 rounded-xl hover:bg-earthy-dark transition font-medium text-sm flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @forelse($categories as $category)
                        <div class="bg-white border border-charcoal/10 rounded-2xl p-5">
                            ...
                            <div class="flex items-center gap-2">
                                <button
                                    @click="editId = {{ $category->id }}; editName = '{{ $category->name }}'; showEditModal = true"
                                ...
                                <form action="{{ route('admin.categories.destroy', $category->id) }}" method="POST"
                                    class="flex-1" onsubmit="return confirm('Yakin ingin menghapus kategori ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="w-full bg-red-500 text-white px-3 py-2 rounded-lg text-sm font-medium hover:bg-red-600 transition">
                                        Hapus
                                    </button>
                                </form>
                            </div>
        ...
        <div class="min-h-screen px-4 flex items-center justify-center">
            ...
            <div class="relative bg-cream rounded-2xl shadow-xl max-w-md w-full p-8 border border-charcoal/10">
                <button @click="showAddModal = false"
                    class="absolute top-4 right-4 text-charcoal/40 hover:text-charcoal transition">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" required
                            class="w-full bg-white border border-charcoal/20 rounded-xl px-4 py-3 text-charcoal focus:outline-none focus:border-earthy focus:ring-2 focus:ring-earthy/20 transition">
                    </div>
                    <button type="submit"
                        class="w-full bg-earthy text-cream py-4 rounded-full hover:bg-earthy-dark transition font-medium text-lg">
                        Simpan Kategori
                    </button>
                </form>
            </div>
    ...
    <div x-show="showEditModal" x-transition class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="min-h-screen px-4 flex items-center justify-center">
            ...
            <div class="relative bg-cream rounded-2xl shadow-xl max-w-md w-full p-8 border border-charcoal/10">
                ...
                <form
                    :action="'{{ route('admin.categories.update', 'placeholder_id') }}'.replace('placeholder_id', editId)"
