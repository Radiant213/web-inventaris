...
<head>
    ...
    <title>Kelola Barang - Admin Inventaris PPLG</title>
...
<body class="bg-cream min-h-screen" x-data="{ sidebarOpen: false, showAddModal: false, showEditModal: false, editItem: null }">
    <div class="flex">
        <!-- Sidebar -->
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" 
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="{{ route('admin.items.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-earthy/10 text-earthy font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 min-h-screen">
            <header class="h-20 bg-white border-b border-charcoal/10 flex items-center justify-between px-6">
                ...
                <h1 class="font-serif text-2xl font-semibold text-charcoal">Kelola Barang</h1>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-cream/50">
                                <tr>
                                    <th class="text-left text-sm font-semibold text-charcoal px-6 py-4">Gambar</th>
                                    <th class="text-left text-sm font-semibold text-charcoal px-6 py-4">Nama Barang</th>
                                    <th class="text-left text-sm font-semibold text-charcoal px-6 py-4">Kategori</th>
                                    <th class="text-left text-sm font-semibold text-charcoal px-6 py-4">Brand</th>
                                    <th class="text-left text-sm font-semibold text-charcoal px-6 py-4">Stok</th>
                            ...
                            <tbody class="divide-y divide-charcoal/10">
                                @forelse($items as $item)
                                    <tr class="hover:bg-cream/30 transition">
                                        <td class="px-6 py-4">
                                            @if($item->image_url)
                                                <img src="{{ $item->image_url }}" alt="{{ $item->name }}" class="w-14 h-14 object-cover rounded-xl">
                                            @else
                                                <div class="w-14 h-14 bg-cream rounded-xl flex items-center justify-center">
                                                    <svg class="w-6 h-6 text-earthy/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                                                    </svg>
                                                </div>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 font-medium text-charcoal">{{ $item->name }}</td>
                                        <td class="px-6 py-4">
                                            <span class="bg-earthy/10 text-earthy px-3 py-1 rounded-full text-sm font-medium">
                                                {{ $item->category->name }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-charcoal/70">{{ $item->brand ?? '-' }}</td>
                                        <td class="px-6 py-4">
                                            <span class="font-semibold {{ $item->stock > 0 ? 'text-green-600' : 'text-red-600' }}">
                                                {{ $item->stock }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                <a href="{{ route('admin.items.edit', $item->id) }}" class="bg-blue-500 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-blue-600 transition">
                                                    Edit
                                                </a>
                                                <form action="{{ route('admin.items.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus barang ini?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="bg-red-500 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-red-600 transition">
                                                        Hapus
                                                    </button>
                                                </form>
                                            </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-12 text-center text-charcoal/60">
    ...
    <div x-show="showAddModal" x-transition class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="min-h-screen px-4 flex items-center justify-center">
            ...
            <div class="relative bg-cream rounded-2xl shadow-xl max-w-lg w-full p-8 border border-charcoal/10">
                ...
                <h3 class="font-serif text-2xl font-semibold text-charcoal mb-6">Tambah Barang Baru</h3>

                <form action="{{ route('admin.items.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="category_id" required class="w-full bg-white border border-charcoal/20 rounded-xl px-4 py-3 text-charcoal focus:outline-none focus:border-earthy focus:ring-2 focus:ring-earthy/20 transition">
                    ...
                    <div>
                        ...
                        <input type="text" name="brand" class="w-full bg-white border border-charcoal/20 rounded-xl px-4 py-3 text-charcoal focus:outline-none focus:border-earthy focus:ring-2 focus:ring-earthy/20 transition">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-charcoal mb-2">Stok *</label>
                        <input type="number" name="stock" min="0" value="0" required class="w-full bg-white border border-charcoal/20 rounded-xl px-4 py-3 text-charcoal focus:outline-none focus:border-earthy focus:ring-2 focus:ring-earthy/20 transition">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-charcoal mb-2">Gambar</label>
                        <input type="file" name="image" accept="image/*" class="w-full bg-white border border-charcoal/20 rounded-xl px-4 py-3 text-charcoal focus:outline-none focus:border-earthy focus:ring-2 focus:ring-earthy/20 transition">
                    </div>
